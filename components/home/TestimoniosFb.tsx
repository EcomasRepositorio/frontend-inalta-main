        ...
        >
          {testimonios.map((testimonio, index) => (
            <SwiperSlide key={index}>
              {/* Flex-grow to adjust height */}
              <div className="flex h-[500px] flex-col items-center p-4 bg-white rounded-lg shadow-lg border border-gray-200">
                <img
                  src={testimonio.foto}
                ...
                <blockquote className="flex flex-col justify-between flex-grow text-center">
                  {/* Text color fixed to gray-800 for visibility */}
                  <p className="text-lg text-gray-800 mb-4">
                    "{testimonio.testimonio}"
                  </p>
                  <footer className="text-gray-600">
                    <cite className="block font-semibold">{testimonio.nombre}</cite>
